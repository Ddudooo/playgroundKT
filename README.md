...
`Map`은 각 키가 고유하고 해당 값을 검색하는 데 사용되는 `Key` / `Value` 쌍의 모음입니다.

맵 생성을 위해 `mapOf ()` 및 `mutableMapOf ()` 함수가 있습니다.

`to infix` 함수를 사용하면 초기화가 덜 시끄 럽습니다.

변경 가능한의 `mutable map` 는 `Map`으로 캐스팅하여 얻을 수 있습니다.

---
`Map <K, V>`는 `Collection` 인터페이스의 상속자가 아닙니다.

그러나 `Kotlin` 컬렉션 유형이기도합니다.

`Map`은 `Key-Value` 쌍 (또는 `entries` )을 저장합니다.

`Key`는 고유하지만 다른 `Key`는 동일한 `Value`으로 쌍을 이룰 수 있습니다.

`Map` 인터페이스는 `Key`를 통한 `Value` 액세스,
`Key` 및 `Value` 검색 등과 같은 특정 기능을 제공합니다.

`MutableMap`은 맵 쓰기 작업이있는 맵입니다.
예를 들어 새 `Key-Value` 쌍을 추가하거나
지정된 `Key`와 관련된 `Value`을 업데이트 할 수 있습니다.

`Map`의 기본 구현 인 `LinkedHashMap`은 `Map`를 반복 할 때 요소 삽입 순서를 유지합니다.
차례로 대체 구현인 `HashMap`은 요소 순서에 대해 아무것도 하지 않습니다.